...
async def play(_, message: Message):
    ...
    if int(chat_id) == int('-1001771245214'):
        await message.reply_text("⚠️ **The @Tenkaimusicx_bot bot in the support group is disabled, to avoid confusion.**\n\n__To avoid spam, you only can safely use it in our group.__",
    ...
    if await is_on_off(1):
        ...
        if int(chat_id) != int(log):
            return await message.reply_text(
               f"😕 Sorry, can't play music for now because it's under maintenance.\n\n• we're sorry for the inconveniences**;** for bug & issues report, reach us in support [chat.](https://t.me/dogsuppor)",
               disable_web_page_preview=True, 
               reply_markup=InlineKeyboardMarkup(
                  [
                     [
                        InlineKeyboardButton("📣 Official Channel",
                        url="https://t.me/Tenkaifams")
                     ]
                  ]
              )
          )
